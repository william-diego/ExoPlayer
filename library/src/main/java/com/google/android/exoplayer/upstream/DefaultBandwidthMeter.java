...
//    void onBandwidthSample(int elapsedMs, long bytes, long bitrate);
    void onBandwidthSample(String label, long startTime, long endTime, int elapsedMs, long bytes, long bitrate);
  ...
  public DefaultBandwidthMeter(String label, Handler eventHandler, EventListener eventListener) {
    this(eventHandler, eventListener, new SystemClock());
    this.label = label;
  }
...
//    long nowMs = clock.elapsedRealtime();
    ...
    if (elapsedMs > 0) {
      ...
      notifyBandwidthSample(startTimeMs, nowMs, elapsedMs, bytesAccumulator, bitrateEstimate);
    }
    streamCount--;
    if (streamCount > 0) {
      startTimeMs = nowMs;
  ...
  private void notifyBandwidthSample(final long startTime, final long endTime, final int elapsedMs, final long bytes, final long bitrate) {
    if (eventHandler != null && eventListener != null) {
      eventHandler.post(new Runnable()  {
        @Override
        public void run() {
//          eventListener.onBandwidthSample(elapsedMs, bytes, bitrate);
          eventListener.onBandwidthSample(label, startTime, endTime, elapsedMs, bytes, bitrate);
        }
      });
    }
  }
